...
const DiagnosisManagementScreen: React.FC = () => {
  ...
  return (
    <Box sx={{ padding: "2rem", backgroundColor: Colors.PrimaryGray, minHeight: "100vh" }}>
      <Typography variant="h4" sx={{ textAlign: "center", mb: 4, color: "#333" }}>
        Gestión de Diagnósticos por Vehículo
      </Typography>
      <SearchBar
            ...
            <TableBody>
              {filteredVehicles.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((vehicle) => (
                <TableRow
                  ...
                  hover
                  sx={{ cursor: "pointer" }}
                  onClick={() => (window.location.href = `/vehicle/${vehicle.idVehicle}/diagnosismanagement`)}
                  >
                  ...
                  <TableCell align="center">{vehicle.diagnosisManager.diagnoses.length}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
...
export default DiagnosisManagementScreen;